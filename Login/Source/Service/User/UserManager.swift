...
final class UserManager: UserManageable {
    ...
    private init(loginKeychainStorage: LoginKeychainStorageable, coreDataManager: CoreDataManager) {
    ...
    func logIn(with user: User) async throws {
        self.user = user
        try await loginKeychainStorage.setUserEmail(with: user.email)
    }
    ...
    func logOut() async throws {
        user = nil
        try await loginKeychainStorage.removeUserEmail()
    }
    ...
    func getPreviousLoggedInUserEmail() async throws -> String {
        let previousUserEmail = try await loginKeychainStorage.getUserEmail()
