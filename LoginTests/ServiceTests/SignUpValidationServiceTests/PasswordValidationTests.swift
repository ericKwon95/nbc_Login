...
struct PasswordValidationTests {
    ...
    @Test("대문자가 없는 비밀번호 테스트", arguments: [
        "test1234!",
        "password1@",
        "nocaps123#"
    ])
    ...
    @Test("특수문자가 없는 비밀번호 테스트", arguments: [
        "Test1234",
        "Password123",
        "NoSpecial1"
    ])
    ...
    @Test("숫자가 없는 비밀번호 테스트", arguments: [
        "TestPass!",
        "Password@",
        "NoNumber#"
    ])
    ...
    @Test("유효하지 않은 길이 비밀번호 테스트", arguments: [
        "Test1!", // 너무 짧음
        "A1!" // 최소 길이 미달
    ])
